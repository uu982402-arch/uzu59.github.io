...
(function(){
  ...
  function initMainWidget(){
    ...
    function render(){
      ...
      // compact summary line (main index)
      const compact = $('fortuneCompactLine');
      if(compact){
        const caution = shortText(data.kCaut || data.ban, 22);
        const reco = shortText(data.mission || data.kMoney, 22);
        compact.innerHTML =
          `<span class="pill">점수 <b>${data.score}</b></span>`+
          `<span class="pill">주의 <b>${escapeHtml(caution)}</b></span>`+
          `<span class="pill">추천 <b>${escapeHtml(reco)}</b></span>`;
      }
