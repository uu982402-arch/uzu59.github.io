...
(function(){
  ...
  function makeFortune(opts){
    ...
    const briefCaution = clipText(ban || kCaut, 18);
    const briefRecommend = clipText(mission, 18);

    return { score, head, keyword, color, number, mission, ban, kMoney, kBody, kLove, kCaut, silly, proList, briefCaution, briefRecommend };
  }
  ...
  function initMainWidget(){
    ...
    function render(){
      ...
      setText('fortuneBriefLine', `점수 ${data.score} · 주의: ${data.briefCaution||'—'} · 추천: ${data.briefRecommend||'—'}`);
    ...
    if(speakBtn){
      speakBtn.addEventListener('click', ()=>{
        ...
        const text = `오늘의 운 브리핑. 점수 ${data.score}. 주의: ${data.briefCaution}. 추천: ${data.briefRecommend}.`;
        speak(text);
      });
    }
    ...
    if(copyBtn){
      copyBtn.addEventListener('click', ()=>{
        ...
        const label = (lvl==='pro'?'프로':(lvl==='risk'?'하이리스크':'초급'));
        const msg = `[88ST 오늘의 운 브리핑] (${signName}/${label})
`+
          `점수 ${data.score} · 주의: ${data.briefCaution} · 추천: ${data.briefRecommend}
`+
