/* 88ST /cert/ v55 patch (2026-02-17)
 * - Fix: rec/new filter now works via tags
 * - Fix: '텍스트 정리' toggle affects popup text (raw vs cleaned)
 * - Add: badge on cards (TOP/추천/신규)
 * - Add: share link copy in popup
 */
(() => {
  'use strict';
  ...
  const CARDS = {
    card1: { title: '어느날', code: 'ST95', link: 'http://oday-147.com', telegram: 'UZU59',
      ...
      notice: '가입코드 미입력 시 혜택 적용 불가', tag: 'new' },
    card2: { title: 'OK Bet', code: '88ST', link: 'https://ok-8888.com/?code=88ST', telegram: 'UZU59',
      ...
      notice: '가입코드 미입력 시 혜택 적용 불가', tag: 'rec' },
  ...
  const badgeFor = (id, card) => {
    if (TOP_SET.has(id)) return { cls: 'top', text: 'TOP' };
    if (card?.tag === 'rec') return { cls: 'rec', text: '추천' };
    if (card?.tag === 'new') return { cls: 'new', text: '신규' };
    return null;
  };
  ...
  const matchCard = (id, q) => {
    ...
    if (filter === 'rec' && c.tag !== 'rec') return false;
  ...
  const renderGrid = () => {
    ...
    grid.innerHTML = list.map((id) => {
      ...
      const badge = badgeFor(id, c);
      const badgeHtml = badge ? `<div class="cert-badge ${badge.cls}">${badge.text}</div>` : '';
      return `
